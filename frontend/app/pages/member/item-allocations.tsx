      ...
      <div className="px-4 pt-4 md:max-w-3xl md:mx-auto">
        ...
        <div className="relative mb-4">
          <input
            ...
            className="w-full border border-[#D9CEB4] rounded-[12px] px-4 py-3 text-[length:var(--fs-base)] text-[#2C2F1E] bg-[#FBF5E8] outline-none focus:border-[#626F47]"
          />
        </div>
        ...
        {isLoading ? (
          ...
          <div
            role="alert"
            className="text-center py-12 p-4 bg-red-50 border border-red-200 rounded-[12px] text-[length:var(--fs-md)] text-red-700"
          >
            {fetchError}
          </div>
          ...
          allocations.map((alloc) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-[length:var(--fs-sm)] text-[#6B7550]">{alloc.date}</div>
              </div>
              <div className="text-right">
                <div className="font-display font-bold text-[length:var(--fs-lg)] text-[#2C2F1E]">
                  {alloc.quantity}
                </div>
                <div className="text-[length:var(--fs-xs)] text-[#6B7550]">{alloc.unit}</div>
              </div>
            </div>
          ))
        )}
